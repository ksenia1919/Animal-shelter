//---------------------------------------------------------------------------

#include <vcl.h>
#pragma hdrstop

#include "Unit1.h"
#include "Unit2.h"
#include "Unit3.h"
#include "Unit4.h"
//---------------------------------------------------------------------------
#pragma package(smart_init)
#pragma resource "*.dfm"
TForm2 *Form2;
//---------------------------------------------------------------------------
__fastcall TForm2::TForm2(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TForm2::FormClose(TObject *Sender, TCloseAction &Action)
{
	Form1->Close();
}
//---------------------------------------------------------------------------

int clik = 0;
void __fastcall TForm2::Image2Click(TObject *Sender)
{

	if(clik == 0){
	Edit2->PasswordChar = false;
	Image2->Picture->LoadFromFile("eye_close.jpg");
	clik++;
	}
	else
	{
	Edit2->PasswordChar = 42; // 42 это номер символа в таблице
	Image2->Picture->LoadFromFile("eye_open.jpg");
	clik--;
	}

}
//---------------------------------------------------------------------------

void __fastcall TForm2::Image4Click(TObject *Sender)
{
	ADOQuery1->Active = false;
	ADOQuery1->Active = true;
	int counter_avtiriz = 0;
	//проверка логина
	if(Edit1->Text!= "")
	{
		TLocateOptions options;
		if (ADOQuery1->Locate("Логин", Edit1->Text, options))
		{
			counter_avtiriz++;
		}
		else
		{
			Application->Title="Опаньки!";
			ShowMessage("Логин не был найден(");
		}
	}
	else if(Edit1->Text == "" && Edit2->Text != "")
	{
	Application->Title="Внимание!";
	ShowMessage("Пожалуйста, введите логин.");
	}
	//проверка пароля
	if(Edit2->Text!= "")
	{
		TLocateOptions options;
		if (ADOQuery1->Locate("Пароль", Edit2->Text, options))
		{
			counter_avtiriz++;
		}
		else
		{
			Application->Title="Опаньки!";
			ShowMessage("Пароль и логин не совпадают");
		}
	}
	else if(Edit1->Text != "" && Edit2->Text == "")
	{
	Application->Title="Внимание!";
	ShowMessage("Пожалуйста, введите пароль.");
	}
	//авторизация
	if(Edit1->Text!= "" && Edit2->Text!= "") {
		if(counter_avtiriz == 2)
		{
			//если у данного пользователя Тип пользователя = Администратор то...
			TLocateOptions options;
			ADOQuery1->Locate("Логин", Edit1->Text, options);
			if(ADOQuery1->FieldByName("Тип пользователя")->AsString == "Администратор"){
            Application->Title="Успех";
			ShowMessage("Вы успешно вошли!");
			Form4->Show();
			Form2->Hide();
			}
			else
			{
			Application->Title="Успех";
			ShowMessage("Вы успешно вошли!");
			Form3->Show();
			Form2->Hide();
			}
		}
	}
	else if( Edit1->Text == "" && Edit2->Text == "")
	{
    Application->Title="Внимание!";
	ShowMessage("Пожалуйста, введите логин и пароль.");
	}

}
//---------------------------------------------------------------------------

void __fastcall TForm2::Image4MouseEnter(TObject *Sender)
{
	Image4->Picture->LoadFromFile("Butto_vhod_Klik.jpg");
}
//---------------------------------------------------------------------------

void __fastcall TForm2::Image4MouseLeave(TObject *Sender)
{
	Image4->Picture->LoadFromFile("Butto_vhod_Active.jpg");
}
//---------------------------------------------------------------------------


void __fastcall TForm2::Image3Click(TObject *Sender)
{
	Form1->Show();
	Form2->Hide();
}
//---------------------------------------------------------------------------

